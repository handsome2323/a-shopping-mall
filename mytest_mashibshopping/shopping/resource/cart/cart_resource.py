...
class Shopping_Cart(Resource):
    ...
    # 操作购物车
    @staticmethod
    def set_Shopping_cart( method, sku_list=[]):
        user_id = g.user_id
        cart_cache_json = redis_client.get('cart_from_user_{}'.format(user_id))   # 查询缓存
        cart_cache_dict = {}
        ...
        if method == 'post':
            ...
            if cart_cache_json:
                ...
                cart_cache_dict = {add_sku_skuNo: add_sku_item}    # 生成新的待缓存的购物车数据
            redis_client.setex('cart_from_user_{}'.format(user_id), 3600 * 48, json.dumps(cart_cache_dict)) # 写入redis
            return add_sku_item
        ...
        if method == 'delete':
            ...
            # delete_sku_item = sku_list[0]  # 暂时写 单条sku删除操作
            ...
            redis_client.setex('cart_from_user_{}'.format(user_id), 3600 * 48, json.dumps(cart_cache_dict))
            return {'msg': 'ok'}
